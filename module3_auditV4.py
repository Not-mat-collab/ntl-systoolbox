...
class NetworkScanner:
    ...
    def _get_hostname(self, ip: str) -> str:
        # 1) Hostname Nmap (le plus fiable)
        try:
    ...
    def _get_os_info(self, ip: str) -> Dict:
        ...
        try:
            ...
            if 'osclass' in self.nm[ip]:
                ...
                if os_classes:
                    ...
                    if class_accuracy >= current_accuracy:
                        ...
                        elif any(kw in os_family_cls for kw in ['linux', 'ubuntu', 'debian',
            ...
            # --- Heuristique via services -sV si family toujours inconnue ---
            if not os_info['os_family']:
    ...
    def _extract_windows_version(self, name: str) -> Optional[str]:
        ...
        # PRIORITÉ 1 : Windows Server (AVANT les build numbers desktop)
        ...
        # PRIORITÉ 2 : Desktop via build number
        if build_number:
...
class OSDetector:
    ...
    def _detect_via_banner(self, ip: str, ports: Dict = None) -> Optional[Dict]:
        common_ports = {
            ...
            3389: 'rdp', 135: 'msrpc', 139: 'netbios', 445: 'smb'
        }
        if ports:
    ...
    def normalize_os_version(self, os_family: str, raw_version: str) -> str:
        ...
        if os_family == 'Windows':
            # Windows Server AVANT desktop
            if 'windows server' in raw_lower or ('server' in raw_lower and 'windows' in raw_lower):
                server_match = re.search(r'server[\s_]?(\d{4})', raw_lower)
                if server_match:
                    return f"Windows Server {server_match.group(1)}"
                return "Windows Server"
            elif 'windows 11' in raw_lower or 'win11' in raw_lower:
                return "Windows 11"
        ...
        return {
            'Windows': {
                ...
                'Windows Server 2025': {
                    ...
                    'eol_date': date(2029, 10, 9),
                    'eol_extended_date': date(2034, 10, 10),
                    'status': 'supported'
                },
                'Windows Server 2022': {
...
class ReportGenerator:
    ...
    def _analyze_components(self, components: List[Dict], eol_database) -> Dict:
        stats = {
            ...
            'supported': 0, 'soon_eol': 0, 'warning': 0,
            'extended_support': 0, 'eol': 0, 'unknown': 0,
            'by_os_family': {}, 'critical': []
        }
        for component in components:
            ...
            if os_family not in stats['by_os_family']:
                stats['by_os_family'][os_family] = {'total': 0, 'supported': 0, 'eol': 0, 'soon_eol': 0}
    ...
    def _generate_csv_report(self, analysis: Dict, components: List[Dict]) -> str:
        ...
        for comp in components:
            ...
            writer.writerow([
                comp.get('ip', 'N/A'), comp.get('hostname', 'N/A'),
                comp.get('os_family', 'Unknown'), comp.get('os_version', 'Unknown'),
                self.status_labels.get(comp.get('status', 'unknown'), comp.get('status', 'unknown')),
                eol_date_str, comp.get('days_until_eol', 'N/A')
            ])
    ...
    def _generate_json_report(self, analysis: Dict, components: List[Dict]) -> str:
        ...
        for comp in components:
            ...
            if eol_info:
                ...
                for key in ['release_date', 'eol_date', 'eol_extended_date']:
                    if key in eol_info_copy and eol_info_copy[key]:
                        eol_info_copy[key] = eol_info_copy[key].isoformat()
...
def scan_network(ip_range: str, output_csv: str = None):
    ...
    for host in hosts:
        ...
        # Correction build number : vérifier Server AVANT desktop
        build_number = os_info_nmap.get('build_number')
        if build_number and os_family == 'Windows':
            os_details_lower = (os_info_nmap.get('os_details') or '').lower()
            if 'server' in os_details_lower:
                if build_number >= 26100:
                    os_version_raw = "Windows Server 2025"
                elif build_number >= 20348:
                    os_version_raw = "Windows Server 2022"
                elif build_number >= 17763:
                    os_version_raw = "Windows Server 2019"
                elif build_number >= 14393:
                    os_version_raw = "Windows Server 2016"
                elif build_number >= 9600:
                    os_version_raw = "Windows Server 2012 R2"
                elif build_number >= 9200:
                    os_version_raw = "Windows Server 2012"
            else:
                if build_number >= 22000:
        ...
        # Heuristique ports si OS toujours inconnu
        ...
        if os_version in ('Linux', 'Windows') and os_version_raw not in ('Unknown', os_version):
            if any(kw in os_version_raw.lower() for kw in ['ubuntu', 'debian', 'centos', 'rhel',
                                                             'red hat', 'server 20', 'server 2008']):
        ...
        status_label = {
            'supported': '[OK]', 'soon_eol': '[WARN]', 'warning': '[WARN]',
            'extended_support': '[EXT]', 'eol': '[EOL]', 'unknown': '[?]'
        }.get(status, '[?]')
...
def menu_list_os():
    ...
    os_families = {'1': 'Windows', '2': 'Linux', '3': 'macOS'}
    if choice in os_families:
        clear_screen()
        try:
            list_os_versions(os_families[choice])
        except Exception as e:
            print(f"\nErreur: {e}")
    elif choice == '4':
        return
...
def menu_process_csv():
    ...
    report_format = {'1': 'txt', '2': 'csv', '3': 'json'}.get(format_choice, 'txt')
    print()
